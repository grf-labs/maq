...
Given the functions $\hat \tau(\cdot)$ and $C(\cdot)$, `maq` delivers a Qini curve that quantifies the value of optimally assigning treatment while satisfying a budget constraint $B$. The policies underlying this treatment allocation are a solution to a series of linear programs, which `maq` solves efficiently.
